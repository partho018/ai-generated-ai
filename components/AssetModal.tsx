    ...
    return (
        <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 24 }}>
            <div onClick={e => e.stopPropagation()} style={{ background: '#1b1b1e', borderRadius: 20, border: '1px solid rgba(255,255,255,0.1)', maxWidth: 520, width: '100%', overflow: 'hidden', boxShadow: '0 40px 80px rgba(0,0,0,0.8)', maxHeight: '90vh', overflowY: 'auto' }}>
                {asset.type === 'video' ? (
                    <video src={asset.url} controls autoPlay loop style={{ width: '100%', aspectRatio: 'auto', objectFit: 'contain', display: 'block', maxHeight: '70vh', background: '#000' }} />
                ) : (
                    <img src={asset.url} alt="" style={{ width: '100%', aspectRatio: 'auto', objectFit: 'contain', display: 'block', maxHeight: '70vh', background: '#000' }} />
                )}
                <div style={{ padding: '16px 20px 20px' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                        <a href={asset.url} download={asset.type === 'video' ? "snapai-video.mp4" : "snapai-image.png"} style={{ flex: 1, padding: '10px', background: '#C6F700', border: 'none', borderRadius: 10, color: '#000', fontWeight: 600, fontSize: 13, cursor: 'pointer', textAlign: 'center', textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>↓ Download</a>
                        {asset.type !== 'video' && (
