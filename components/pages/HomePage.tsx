...
import { IMAGE_MODELS, getAutoQuality } from '@/lib/models';
...
export default function HomePage({ allAssets, setAllAssets, showMsg, onGoToAssets, imageRefs, setImageRefs, imageModel, setImageModel, imageAspect, setImageAspect, imageCount, setImageCount }: HomePageProps) {
    ...
    const autoQuality = getAutoQuality(imageModel);
    const activeModel = MODELS.find(m => m.id === imageModel) ?? MODELS[0];
    const maxCount = activeModel.maxCount;

    const closeAll = () => { setShowHomeModelPicker(false); setShowAspectPicker(false); };
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            console.log("🎨 Generation SUCCESS! Photo link received:", data.images[0]);
            ...
            showMsg(`${data.images.length} image${data.images.length > 1 ? 's' : ''} created! ✨`, 'success');
        } catch (err: any) {
                ...
                <div style={{ maxWidth: 840, margin: '0 auto', position: 'relative' }}>
                    ...
                    <div style={{ background: '#111113', border: `1px solid ${prompt.trim() && !isSafePrompt(prompt) ? 'rgba(239,68,68,0.5)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 24, padding: '12px', boxShadow: '0 24px 60px rgba(0,0,0,0.6)', transition: 'all 0.3s' }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            {showAspectPicker && (
                                <div style={{ position: 'absolute', bottom: '115%', left: 0, background: '#16161a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 8, zIndex: 200, minWidth: 210, boxShadow: '0 16px 48px rgba(0,0,0,0.6)', marginBottom: 6 }}>
                                    ...
                                    {ASPECTS.map(a => (
                                        <div key={a.id}
                                            onClick={() => { setImageAspect(a.id); setShowAspectPicker(false); }}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 8px 6px', gap: 6, flexWrap: 'nowrap', overflowX: 'auto' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'nowrap', position: 'relative' }}>

                                    {/* Model Selector */}
                                    <div style={{ position: 'relative' }}>
                                        <button onClick={() => { setShowHomeModelPicker(v => !v); setShowAspectPicker(false); }} style={pill(showHomeModelPicker)}>
                                            <span style={{ fontSize: 13 }}>{activeModel.icon}</span>
                                            {activeModel.label}
                                            <span style={{ opacity: 0.3 }}>›</span>
                                        </button>
                                        {showHomeModelPicker && (
                                            <div style={{ position: 'absolute', bottom: '115%', left: 0, background: '#16161a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: 8, zIndex: 200, minWidth: 300, boxShadow: '0 20px 50px rgba(0,0,0,0.6)', marginBottom: 6 }}>
                                                ...
                                                {MODELS.map(m => (
                                                    <div key={m.id}
                                                        onClick={() => { setImageModel(m.id); setImageCount(Math.min(imageCount, m.maxCount)); closeAll(); }}
                                                        ...
                                                        <div style={{ width: 40, height: 40, borderRadius: 11, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 19, flexShrink: 0 }}>{m.icon}</div>
                                                        <div style={{ flex: 1, minWidth: 0 }}>
                                                            <div style={{ fontSize: 13, fontWeight: 700, color: '#fff', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                                                                {m.label}
                                                                {m.badge && <span style={{ fontSize: 8, fontWeight: 800, background: m.badgeColor, color: '#fff', padding: '1px 5px', borderRadius: 4 }}>{m.badge}</span>}
                                                            </div>
                                                            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.38)', display: 'flex', alignItems: 'center', gap: 8 }}>
                                                                <span>{m.desc}</span>
                                                                <span style={{ background: 'rgba(198,247,0,0.1)', border: '1px solid rgba(198,247,0,0.2)', color: '#C6F700', fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 4 }}>{m.autoQuality.label}</span>
                                                            </div>
                                                        </div>
                                    ...
                                    {/* Aspect ratio */}
                                    <button onClick={() => { setShowAspectPicker(v => !v); setShowHomeModelPicker(false); }} style={pill(showAspectPicker)}>
